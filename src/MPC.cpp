 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      // make cost of steering and acceleration at the same time higher
      fg[0] += 1000*CppAD::pow(vars[delta_start + i] * vars[v_start+i], 2);
    }

    // cost of change of steering and a, make if smoother
    for (int i = 0; i < N - 2; i++) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      //if t>1 take in account latency for actuators
